...
- Billing invoices used by this feature must always be filtered by the current authenticated `user_id`
...
- Every DAS calculation must belong to the authenticated user through `user_id`
- New DAS calculations created by the application must persist the current authenticated `user_id`
- Reads, timeline lookups, stored month lookups, and list queries must be scoped to the current authenticated `user_id`
...
- The calculation must use only invoices owned by the current authenticated user
...
- Results must include only DAS calculations owned by the current authenticated user
...
- The operation must behave as not found when the DAS calculation does not belong to the current authenticated user
...
- The operation must behave as not found when the tax breakdown belongs to another user's DAS calculation
...
- Stored month lookups and preview invoices must be scoped to the current authenticated user
...
- Revenue must come only from billing invoices owned by the current authenticated user
...
- Ownership checks must be enforced before exposing or mutating stored DAS records by identifier
...
- DAS calculations, timeline items, and stored month lookups only use the current authenticated user's data
- One authenticated user can not read or mutate another user's persisted DAS records
